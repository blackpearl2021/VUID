...
% noisyDirName='D:\Continued from 2019\US despeckling experiment\2023 VUID\RealTrainingSetBUID\noisyImages';
% VUIDnetOutDirName='D:\Continued from 2019\US despeckling experiment\2023 VUID\RealTrainingSetBUID\VUIDnetOutImages';

noisyDirName='D:\Continued from 2019\US despeckling experiment\2023 VUID\RealTrainingSetBUSI\noisyImages';
VUIDnetOutDirName='D:\Continued from 2019\US despeckling experiment\2023 VUID\RealTrainingSetBUSI\VUIDnetOutImages';
...
%  epsilon1=0.5e-1;%variance=5e-3
 epsilon1=2.236e-2;%variance=1.0e-3
% epsilon1=1.581e-2;%variance=0.5e-3
% epsilon1=7.07-3;%variance=1.0e-4
% epsilon1=2.236e-3;%variance=1.0e-5
%epsilon1=0.5e-3;%variance=0.5e-6
% epsilon1=1.581e-3;%variance=5.0e-6;sqrt(2.5)=1.581
% epsilon1=7.07e-4;%variance=1.0e-6
% epsilon1=7.07e-5;%variance=1.0e-8
p=7;
alpha0=0.5*p*p;
...
% function [ValidationLoss,ValidationPSNR] = modelPredictions(DespecklingNet,SigmaSqNet,mbq)
% 
% ValidationLoss = [];
% ValidationPSNR = [];
% 
...
%     [X,T] = next(mbq);
% 
%     % Calculate the predictions for the DenoiserNet, EncoderNet, DecoderNet, and EstimatorNet.
%     LogSigmaSq = predict(SigmaSqNet,X);
% 
%     % Spatially stretch the Sigma map
%     SigmaSq=exp(LogSigmaSq);
%     [nx,ny,~,nb]=size(X);
%     SigmaMap=sqrt(SigmaSq).*ones(nx,ny,1,nb);
% 
%     % Calculate the predictions for the DespecklingNet, and SigmaSqNet.
%     LogMiu = predict(DespecklingNet,X,SigmaMap);
% 
%     % Adopt reparameterization trick to generate Z
%     Miu=exp(LogMiu);
%     epsi=randn(size(Miu));
%     Z=epsi.*sqrt(1.0e-6)+Miu;
%     Z(Z<0)=1.0e-8;
% 
% 
%     % Calculate the loss including likelihood loss (LH_loss), KL_Miu loss (KL_Miu), and KL_SigmaSq loss.
%     LH_loss=-0.5*log(2*pi)-0.5*log(Z)-0.5*(log(50*SigmaSq)-3.8816)-0.5*49./(50*SigmaSq).*(X-Z).^2./Z;
%     KL_Miu=0.5*(T(:,:,1:3,:)-Miu).^2./(1.0e-6);
%     KL_SigmaSq=49*(log(SigmaSq)-log(T(:,:,4,:)))+49*(T(:,:,4,:)./SigmaSq)-49;
%     lossVUID=mean(KL_SigmaSq,"all")+mean(KL_Miu,"all")-mean(LH_loss,"all");
%     PSNR=-10*log((T(:,:,1:3,:)-Miu).^2);
%     PSNR=mean(PSNR,"all");
% 
%     ValidationLoss = cat(2,ValidationLoss, lossVUID);
%     ValidationPSNR = cat(2,ValidationPSNR, PSNR);
% 
% end
% 
% end
